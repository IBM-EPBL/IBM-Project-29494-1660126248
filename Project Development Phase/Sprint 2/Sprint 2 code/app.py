...
app.config['hostname'] = '1bbf73c5-d84a-4bb0-85b9-ab1a4348f4a4.c3n41cmd0nqnrk39u98g.databases.appdomain.cloud'
app.config['port'] = '32286'
app.config['protocol'] = 'tcpip'
app.config['uid'] = 'wwb84730'
app.config['pwd'] = 'O2rrLro2Lm1JVBKx'
app.config['security'] = 'SSL'
try:
    mysql = DB2(app)

    conn_str='database=bludb;hostname=1bbf73c5-d84a-4bb0-85b9-ab1a4348f4a4.c3n41cmd0nqnrk39u98g.databases.appdomain.cloud;port=32286;protocol=tcpip;\
...
def addexpense():
    ...
    param = "SELECT * FROM expenses WHERE userid = " + "\'" + session['id'] + "\' AND MONTH(date) = MONTH(current timestamp) AND YEAR(date) = YEAR(current timestamp) ORDER BY date DESC"
    res = ibm_db.exec_immediate(ibm_db_conn, param)
    dictionary = ibm_db.fetch_assoc(res)
    expense = []
    while dictionary != False:
    ...
    param = "SELECT userid, limitss FROM limits WHERE userid =  " + "\'" + session['id'] + "\' ORDER BY limitss DESC LIMIT 1"
    res = ibm_db.exec_immediate(ibm_db_conn, param)
    dictionary = ibm_db.fetch_assoc(res)
    row = []
    s = 0
...
@app.route("/today")
def today():
      param1 = "SELECT TIME(date) as tn, amount FROM expenses WHERE userid = " + "\'" + session['id'] + "\'" #AND DATE(date) = DATE(current timestamp) ORDER BY date DESC"
      ...
      param = "SELECT * FROM expenses WHERE userid = " + "\'" + session['id'] + "\'"  # AND DATE(date) = DATE(now()) ORDER BY date DESC"
      res = ibm_db.exec_immediate(ibm_db_conn, param)
      dictionary = ibm_db.fetch_assoc(res)
      print(dictionary)
      expense = []
...
def month():
      ...
      param = "SELECT * FROM expenses WHERE userid =  " + "\'" + session['id'] + "\'" #AND MONTH = MONTH(current timestamp) AND YEAR = YEAR(current timestamp) ORDER BY date DESC
...
def year():
      ...
      param1 = "SELECT * FROM expenses WHERE userid = " + "\'" + session['id'] + "\'" #AND YEAR(date) = YEAR(current timestamp) GROUP BY MONTH(date) ORDER BY MONTH(date)
      res = ibm_db.exec_immediate(ibm_db_conn, param1)
      dictionary = ibm_db.fetch_assoc(res)
      print(dictionary)
      expense = []
      while dictionary != False:
      ...
      return render_template("today.html", texpense = total, expense = expense,  total = total ,
                           t_food = t_food,t_entertainment =  t_entertainment,
                           t_business = t_business,  t_rent =  t_rent, 
                           t_EMI =  t_EMI,  t_other =  t_other )
